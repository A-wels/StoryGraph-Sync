...
def main():
    # get arguments: Path to moon+ reader cache files,
    # storygraph username and password
    parser = argparse.ArgumentParser()
    parser.add_argument(
        '-p', '--path', help='path \
            to moon+ reader cache files', default="./Cache", nargs='?')
    ...
    logging.basicConfig(
        level=args.loglevel, format='[INFO] [%(asctime)s]: %(message)s',
        datefmt='%H:%M:%S')
        ...
        with open(args.path + "/books.txt", "r") as f:
            for line in f:
                for b in books:
                    ...
                    if line.strip().startswith(b.name) and\
                            (float(b.progress) != progress or b.isFinished != finished):
