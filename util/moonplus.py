...
def get_moonplus_books(path: str) -> List[Book]:
    ...
    # get list of books from moon+ reader cache files. Each book has a .po
    # file at the path
    books = []
    for f in os.listdir(path):
        if f.endswith('.po'):
            ...
            name = f.split('.')[0]
            # get progress from .po file: Progress in percent
            # after : in first line
            with open(os.path.join(path, f), 'r') as file:
                progress = float(file.readline().split(':')[1].split('%')[0])

            # check if a book is already in the list.
            # If so, update progress if it is higher
            found = False
            for b in books:
                if b.name == name:
                    found = True
...
def _download_from_nextcloud(cache_path: str) -> None:
    logging.info("Downloading from Nextcloud...")
    with NextCloud(endpoint=config.NEXTCLOUD_URL,
        ...
        for f in folders.data:
            if (f.href.endswith('.po')):
                # Format:
                # /remote.php/dav/files/alex/Apps/Books/.Moon+/Cache/Alice_de.epub.po
